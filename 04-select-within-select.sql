...
/* 4.  Which country has a population that is more than United Kingdom but 
...
SELECT name, CONCAT(CAST(
  ROUND((population/(SELECT population FROM world WHERE name = 'Germany')), 2) * 100 AS int), '%')
FROM world WHERE continent = 'Europe'

/* 6. Which countries have a GDP greater than every country in Europe? 
[Give the name only.] (Some countries may have NULL gdp values) */
SELECT name FROM world a
WHERE gdp > ALL (SELECT gdp FROM world b WHERE continent = 'Europe')
...
FROM world a
WHERE name = (SELECT b.name FROM world b WHERE a.continent = b.continent ORDER BY b.name LIMIT 1)
...
/* 10. Some countries have populations more than three times that of all of their 
neighbours (in the same continent). Give the countries and continents. */
SELECT name, continent FROM world a
  WHERE population > ALL (SELECT 3 * population FROM world b WHERE a.continent = b.continent AND a.name <> b.name AND population > 0)